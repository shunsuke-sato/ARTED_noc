...
!
subroutine init_Ac_basis_expansion_Houston_probe_decomp
...
! pump laser
    do iter=0,Nt+2
      ...
      if (tt<tpulse_1) then
        ...
        Ac_pump_BE(iter)=-f0_1/omega_1*(cos(pi*(tt-0.5d0*tpulse_1)/tpulse_1))**2&
            *sin(omega_1*(1d0+chirp_1*(tt-0.5d0*tpulse_1))*(tt-0.5d0*tpulse_1)+phi_CEP_1*2d0*pi)
      end if
    enddo
! probe laser
    do iter=0,Nt+2
      ...
      if ( (tt>0.5d0*(tpulse_1-tpulse_2)+T1_T2) .and. (tt>0.5d0*(tpulse_1-tpulse_2)+T1_T2+tpulse_2) ) then
        Actot_BE(iter)=Actot_BE(iter) &
          &-f0_2/omega_2*(cos(pi*(tt-(0.5d0*tpulse_1+T1_T2))/tpulse_2))**2&
...
! pump laser
    do iter=0,Nt+2
      ...
      if (tt<tpulse_1) then
        ...
        Ac_pump_BE(iter)=-f0_1/omega_1*(cos(pi*(tt-0.5d0*tpulse_1)/tpulse_1))**4&
            *sin(omega_1*(1d0+chirp_1*(tt-0.5d0*tpulse_1))*(tt-0.5d0*tpulse_1)+phi_CEP_1*2d0*pi)
...
end subroutine init_Ac_basis_expansion_Houston_probe_decomp
