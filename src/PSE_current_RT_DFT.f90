...
!
subroutine PSE_current_RT_DFT(jav)
...
!    curr_l(1)=curr_l(1)+occ(ib,ik)*(-sum(conjg(zft3(:,:,:))*Grad_x_zI(:,:,:)*zft1(:,:,:))+kAc_Cvec(1,ik))/dble(NL1*NL2*NL3)
!    curr_l(2)=curr_l(2)+occ(ib,ik)*(-sum(conjg(zft3(:,:,:))*Grad_y_zI(:,:,:)*zft1(:,:,:))+kAc_Cvec(2,ik))/dble(NL1*NL2*NL3)
!    curr_l(3)=curr_l(3)+occ(ib,ik)*(-sum(conjg(zft3(:,:,:))*Grad_z_zI(:,:,:)*zft1(:,:,:))+kAc_Cvec(3,ik))/dble(NL1*NL2*NL3)
...
!    jzt=jx3t
  ...
  return
end subroutine PSE_current_RT_DFT
