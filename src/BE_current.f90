...
!== construct current matrix end
  ...
  Band : do ib=1,NB_TD
    ...
    zACt_tmp(:) = matmul(zPi_tot(:,:,ik,1),zCt(:,ib,ik))
    jav_l(1) = jav_l(1) + occ(ib,ik)*sum(conjg(zCt(:,ib,ik))*zACt_tmp(:))

    zACt_tmp(:) = matmul(zPi_tot(:,:,ik,2),zCt(:,ib,ik))
    jav_l(2) = jav_l(2) + occ(ib,ik)*sum(conjg(zCt(:,ib,ik))*zACt_tmp(:))

    zACt_tmp(:) = matmul(zPi_tot(:,:,ik,3),zCt(:,ib,ik))
    jav_l(3) = jav_l(3) + occ(ib,ik)*sum(conjg(zCt(:,ib,ik))*zACt_tmp(:))
